<!-- i18n-sync: README.md @ 6c3926a -->
<!-- i18n-sync-date: 2026-05-09 -->
...
**核心理念：统一接口，多后端实现。** 调用方不需要关心目标主机用的是蓝牙还是 USB，只需要通过同一套 API 发送"按 A 键"、"推左摇杆"等指令。
...
| Nintendo Switch | USB（Gadget） | Pro Controller | 已实现 |
...
蓝牙后端通过 D-Bus 与 BlueZ 通信，依赖 `python3-dbus` 这个 C 扩展。该模块**无法通过 pip 安装**——必须先用 apt 装好系统包，再用 `--system-site-packages` 创建 venv，这样 venv 才能看到它：
...
如果创建手柄时报 `No module named 'dbus'`，说明 venv 创建时没加 `--system-site-packages`。按上面的命令重建，或编辑 `.venv/pyvenv.cfg` 把 `include-system-site-packages` 改为 `true`。
...
Switch 走**蓝牙**时需先进入「更改握法/顺序」菜单（设置 > 手柄和传感器 > 更改握法/顺序），然后调用：
...
Switch 走 **USB** 时，将树莓派的 gadget 端口用 USB 数据线连到 Switch 底座，然后：
...
`transport` 字段可选，默认 `"bluetooth"`。USB 模式下数据线可以一直插着——`connect` / `DELETE` 操作通过 UDC 绑定/解绑做软连接和软断开，Switch 底座感知到的是插拔事件。
...
├── usb/                     # USB Gadget HID 协议栈
│   ├── gadget.py            # ConfigFS gadget 配置 + UDC 绑定/解绑
│   ├── hid_device.py        # /dev/hidg0 读写封装
│   ├── switch_protocol.py   # USB 专用 0x80 握手（扩展自蓝牙协议）
│   └── constants.py         # VID/PID、HID report descriptor、USB 命令码
├── backends/
│   ├── switch.py            # SwitchBackend（蓝牙）
